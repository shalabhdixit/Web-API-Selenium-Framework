...
public class ExecutionContext {
	...
	public void addResult(String testcase, String desc, String expected, String actual, byte status) {
		// Based on status, console information will display--Raj added on
		// 06/15/2016--start
		if (status == 1) {
			...
			status = 1;// When I pass status as 2, result was displaying in red and was counting as
						// fail, hence setting as status =1
			System.out.println("*** Test Step INFO: " + testcase);
		}
//		--end---
		this.mongoDB.addResultToTest(this.strTestRunId, this.strScenarioRunId,
				GenerateKey.GenerateExecutionId("RESULT"), this.strScenarioName, testcase, desc, expected, actual,
				status, null);
	}
...
//		--end---
		this.mongoDB.addResultToTest(this.strTestRunId, this.strScenarioRunId,
				GenerateKey.GenerateExecutionId("RESULT"), this.strScenarioName, testcase, desc,
				Integer.toString(expected), Integer.toString(actual), status, null);
	}
...
//		--end---
		this.mongoDB.addResultToTest(this.strTestRunId, this.strScenarioRunId,
				GenerateKey.GenerateExecutionId("RESULT"), this.strScenarioName, testcase, desc,
				expected ? "true" : "false", actual ? "true" : "false", status, null);
	}
...
//		--end---
	...
	public void addResult(String testcase, String desc, int expected, int actual, byte status,
		...
		this.mongoDB.addResultToTest(this.strTestRunId, this.strScenarioRunId,
				GenerateKey.GenerateExecutionId("RESULT"), this.strScenarioName, testcase, desc,
				Integer.toString(expected), Integer.toString(actual), status, screenshotFileName);
	}
	...
	public void addResult(String testcase, String desc, boolean expected, boolean actual, byte status,
		...
		this.mongoDB.addResultToTest(this.strTestRunId, this.strScenarioRunId,
				GenerateKey.GenerateExecutionId("RESULT"), this.strScenarioName, testcase, desc,
				expected ? "true" : "false", actual ? "true" : "false", status, screenshotFileName);
	}
