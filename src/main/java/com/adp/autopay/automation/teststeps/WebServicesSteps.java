...
import com.adp.autopay.automation.commonlibrary.AbstractComparator;
import com.adp.autopay.automation.commonlibrary.CommonUtility;
import com.adp.autopay.automation.commonlibrary.DataHelper;
import com.adp.autopay.automation.commonlibrary.DataModel;
import com.adp.autopay.automation.commonlibrary.DataObject;
import com.adp.autopay.automation.commonlibrary.FileOperations;
import com.adp.autopay.automation.commonlibrary.JSONCompareResult;
import com.adp.autopay.automation.commonlibrary.JSONData;
import com.adp.autopay.automation.commonlibrary.JSONUtility;
import com.adp.autopay.automation.commonlibrary.ListModel;
...
public class WebServicesSteps
{
	...
	//WebServiceParameters webServiceParameters = new WebServiceParameters(null);
	ExecutionContext context;
	List<DataObject> dataObjects;
	
	WebServiceParameters wbServiceParameters = new WebServiceParameters();
	...
	public void ReadAndProcessJsonInputFile(String propertiesFileKey, String SucessorFailure, String status, String UpdatePayload){
		try{
			...
			//This is test name in testNG xml. So test name should be ended with 'feature' keyword
			...
			if (j == -1)
			{
				...
				System.err.println("Please check test name tag in testNG xml, it is not in correct format, 'feature' is missing: "+context.STestName);
			}
			String testname = "";
			...
			for (DataObject obj : dataObjects) {
				...
				if(index != -1){
					...
					String strURL = webServiceParameters.getStrURI();
					
					URL surl = new URL(strURL);
					URI uri = new URI(surl.getProtocol(), surl.getUserInfo(), surl.getHost(), surl.getPort(), surl.getPath(), surl.getQuery(), surl.getRef());
					
					
					//String strUri = "http://"+uri.getHost()+":"+uri.getPort()+uri.getPath()+"?"+URLEncoder.encode(uri.getQuery(), "UTF-8");
					
					if(strURL.contains("{ITEMID}")){
						String strItemID = webServiceParameters.getItemsIDs(obj.getUrlParams(), obj.getBaseUrl());
						if(strItemID!=null){
							strURL = URLEncoder.encode(uri.getQuery(), "UTF-8");
							strURL = strURL.replace("%24", "$").replace("%3D", "=").replace("%26", "&");
							strURL = strURL.replace("%7BITEMID%7D", strItemID.replace("+", "%2B").replace("&", "%26"));
							Field field = URI.class.getDeclaredField("query");
							field.setAccessible(true);
							field.set(uri, strURL);
							field = URI.class.getDeclaredField("string");
							field.setAccessible(true);
							field.set(uri, null);
							//strURL = strURL.toString().replace("{ITEMID}", strItemID);
						}else
						{
							this.context.addResult("Execute webservice call for "+propertiesFileKey + " For DataRow_" + (actualObj.getRowNum()+1), "Execute webservice call: "+ webServiceParameters.getStrTestName(), "Verify the Item id is present", "ItemID is not Found for Query parameters :" +obj.getUrlParams().toString(), RunStatus.FAIL);
							return;
						}
					}
					...
					if(webServiceParameters.getStrRequestType().equals("GET")){
						if(webServiceParameters.getHeaders() !=null){
							...
							res = given().urlEncodingEnabled(false).headers(new Headers(webServiceParameters.getHeaders())).get(uri);
						}else
							res =  given().get(strURL);
					}
					////Code added by Prabhakar////////////////////////////////////////////////
						...
						if(webServiceParameters.getHeaders() !=null){
							...
							res = given().headers(new Headers(webServiceParameters.getHeaders())).contentType("application/json").delete(strURL);
						}else
							res =  given().get(strURL);
					}
                   else{
					...
					/***Code added by Bhanuprakash to attach json file to email***/
					DataHelper.strFileName=strJSONOutputFileName.toString();
					DataHelper.lstrTempFileName.add(DataHelper.strFileName);
					/************************************************************/
					
					String[] Invalidcodelist = {"400","404","500","204","409", "403"};
					if(strJsonOutput.trim().length() >0 )
					{
						writeLineToFile(strJSONOutputFileName, strJsonOutput);
					}
					OutputFileMapping(("ROW"+(actualObj.getRowNum()+1)).toString(), strJSONOutputFileName);
	...
	public void attachJsonFile(){
		DataHelper.blntempEmailJsonResponse = true;
	}
	public static void writeLineToFile(String strFileName, String strData)
	{
		try {
			...
			if(strData != ""){
				...
				IOUtils.write(strData, fos, Charset.defaultCharset());
			}
			else{
	...
	public void ProcessJsonInputFileWithExcelALL(String propertiesFileKey, String SucessorFailure, String status, String UpdatePayload ,String MyInputDataSheet , String RowIndex , String[] Values , String ServiceName){
		try{
			...
			if(j != -1){
				...
				Response res = null;
				//URL surl = new URL(webServiceParameters.getStrURI());
				//URI uri = new URI(surl.getProtocol(), surl.getUserInfo(), surl.getHost(), surl.getPort(), surl.getPath(), surl.getQuery(), surl.getRef());
				String strURL = webServiceParameters.getStrURI();
				testname =  webServiceParameters.getStrTestName();
				if(webServiceParameters.getStrRequestType().equals("GET")){
					if(webServiceParameters.getHeaders() !=null){
						...
						res = given().headers(new Headers(webServiceParameters.getHeaders())).get(strURL);
					}else
						res =  given().get(strURL);
				}
				////Code added by Prabhakar////////////////////////////////////////////////
				...
				if (webServiceParameters.getStrRequestType().equals("POST")) {
					...
					if(webServiceParameters.getHeaders() !=null) {
						...
						System.out.print(webServiceParameters.getHeaders());
						//System.out.print(webServiceParameters.getStrURI());
						res = given().headers(new Headers(webServiceParameters.getHeaders())).contentType("application/json").body(strBody).post(strURL);
					}
					else
						res =  given().contentType("application/json").post(strURL, strBody);
					...
					if(webServiceParameters.getHeaders() !=null)
						res = given().headers(new Headers(webServiceParameters.getHeaders())).contentType("application/json").body(strBody).put(strURL);

					else
						res =  given().contentType("application/json").put(strURL, strBody);
				...
				System.out.println("Status Code= "+ res.getStatusCode());
				System.out.println("Status Line= "+res.getStatusLine());
				//===Added by Raj on 6/15/2016=====start==Even the request was having 400, test used to display as Test Step Passed. Now changed to display fail if there is 400
				if(res.getStatusCode()==200 && res.getStatusLine().equalsIgnoreCase("HTTP/1.1 200 OK")){
					this.context.addResult("Executed webservice call for "+propertiesFileKey +" For Data Row: "+RowIndex, "Execute webservice call: "+ webServiceParameters.getStrTestName(), "Webserivce call executed ", "Webservice call returned Status Code = " + res.getStatusCode() +  " StatusLine =  " + res.getStatusLine(), RunStatus.PASS);

				}else if(res.getStatusCode()==400 && res.getStatusLine().equalsIgnoreCase("HTTP/1.1 400 Bad Request")){
					this.context.addResult("Executed webservice call for "+propertiesFileKey +" For Data Row: "+RowIndex, "Execute webservice call: "+ webServiceParameters.getStrTestName(), "Webserivce call executed ", "Webservice call returned Status Code = " + res.getStatusCode() +  " StatusLine =  " + res.getStatusLine(), RunStatus.FAIL);
...
//				======end====
	...
	@Then("^WEBSERVICES:Verify Response Fields as Values with hierarchy for JSON \"([^\"]*)\"$")
	public void VerifyResponseJSONForFieldsAndValuesWithHierarcy(String JSONFile ,DataTable table ) {
		...
		if(index == -1) {
			this.context.addResult("Reading data file for verify Field and values based on Hierarcy ", "Reading data file ", "Invalid Service Name " + JSONFile, " Serivce Name does not exists in data file to verify the Field and values with hierarcy",  RunStatus.FAIL);
			System.exit(0);
		}
		...
		try {
			...
			if (j == -1)
			{
				this.context.addResult("Reading webservice property file name for verify Field and values based on Hierarcy ", "Reading webservice property file name ", "Invalid user story name " , "Invalid user story name ",  RunStatus.FAIL);
			}
			String USname = context.STestName.substring(0,j);
			...
			if(file.exists()){
				...
				for (int t = 0; t < data.size(); t++) {
						...
						Field = data.get(t).get("Field").trim();
						Value = data.get(t).get("Value").trim();
						String strMissmatchValue = wbServiceParameters.verifyFieldNValwithHiearcy(content,Field,Value);
						if (strMissmatchValue.trim().length()==0) {
							System.out.println("Field Value verification with Hierarchy, For Field: "+Field + "Expected value :" + Value + "is found");
							this.context.addResult("Field Value verification with Hierarchy  " + JSONFile + ": " + "for " + Field + "=" + Value + " for " + "_DataRow_" + (actualObj.getRowNum()+1), "Field Value verification with Hierarchy" + JSONFile, Field + "=" + Value," Field Value verification with Hierarchy, For Field: "+Field + "Expected value :" + Value + "is found.", RunStatus.PASS);
						} else {
							System.out.println("Field Value verification with Hierarchy, For Field: "+Field + "Expected value :" + Value + "is not found. Actual Values :" +strMissmatchValue);
							this.context.addResult("Field Value verification with Hierarchy  " + JSONFile + ": " + "for " + Field + "=" + Value + " for " + "_DataRow_" + (actualObj.getRowNum()+1), "Field Value verification with Hierarchy" + JSONFile, Field + "=" + Value," Field Value verification, For Field: "+Field + "Expected value :" + Value + "is not found. Actual Values :" +strMissmatchValue, RunStatus.FAIL);}
					}
				fis.close();
				}
				...
				this.context.addResult("Verify Filed and Values in Response of the WebService", "Verify Filed and Values in Response of the WebService"+JSONFile,"Verify the output JSON File existance" , "JSON output File not found", RunStatus.FAIL);
			}
		}
			}
		}
		...
		try {
			...
			if(file.exists()){
				...
				/*	content = content.replaceAll("\\r\\n|\\r|\\n", "");
					...
					//while ((content != null)) {
							String[] Fields = content.replace("{", "").replace("}", "").replace("]", "").replace("[","").replace(",\"", "@#").split("@#"); 
							for (int i = 0; i < Fields.length; i++) {
								if (Fields[i].contains(Field)) {
									String Line = Fields[i];
									int start = (Line.indexOf(Field) + (Field.length()) + 2);
									int end = Line.length();
									String outputValue = (Line.substring(start, end).trim().replace("\"", "")).replace("\\", "");
...
*/
					...
					String strMissmatchValue = wbServiceParameters.verifyJsonTagNValue(json, Field, Value.trim());
					
					fis.close();
						if (strMissmatchValue.length()> 0) {
					//if(m>0){
							System.out.println("Field Value verification, For Field: "+Field + "Expected value :" + Value + "is not found. Actual Values :" +strMissmatchValue);
							this.context.addResult("Verify Response for the WebService  " + JSONFile + ": " + "for " + Field + "=" + Value + " for " + "_DataRow_" + (actualObj.getRowNum()+1), "Verify Response for the WebService" + JSONFile, Field + "=" + Value," Field Value verification, For Field: "+Field + "Expected value :" + Value + "is not found. Actual Values :" +strMissmatchValue, RunStatus.FAIL);
					/*	System.out.println("Successfully Verified Response JSON file for " + JSONFile + ": " + " for " + Field + "=" + Value + " And" + " There Are " + m + " Occurrences of the  " + "\n" + Field + "=" + Value);
						this.context.addResult("Verify Response for the WebService  " + JSONFile + ": " + "for " + Field + "=" + Value + " for " + "_DataRow_" + (actualObj.getRowNum()+1), "Verify Response for the WebService  " + JSONFile + ": " + "for " + Field + "=" + Value, Field + "=" + Value, "Verified Response For  " + Field + "=" + Value + " And" + " There Are " + m + " Occurrences of the " + Field + "=" + Value, RunStatus.PASS); */
							...
							System.out.println("Field Value verification, For Field: "+Field + "Expected value :" + Value + "is found");
							this.context.addResult("Verify Response for the WebService  " + JSONFile + ": " + "for " + Field + "=" + Value + " for " + "_DataRow_" + (actualObj.getRowNum()+1), "Verify Response for the WebService" + JSONFile, Field + "=" + Value," Field Value verification, For Field: "+Field + "Expected value :" + Value + "is found.", RunStatus.PASS);
...
public void verifyResponseFile( String ErrorType , String JSONFile , String FilePath , String FieldToVerify , List<String> ListToVerify )
{
	...
	File file = new File(FilePath);
				...
				if(file.exists()){
					...
					//while ((content != null)) {
							...
							for (int i = 0; i < Fields.length; i++) {
								if (Fields[i].contains(Field)) {
									...
									int start = (Line.indexOf(Field) + (Field.length()) + 2);
									int end = Line.length();
									outputValue = (Line.substring(start, end).trim().replace("\"", ""));
									if (outputValue.equalsIgnoreCase(Value)) {
										m = m + 1;
									} else {
										n = n + 1;
					...
					{
						if (m > 0) {
							System.out.println("Successfully Verified " + ErrorType + " Error Descriptions in Response JSON file for " + JSONFile + ": " + " for " + " Expected Error Description In Input JSON" + "=" + Value );
							System.out.println("Successfully Verified " + ErrorType + " Error Descriptions in Response JSON file for " + JSONFile + ": " + " for " + " Actual Error Description In Output JSON" + "=" + outputValue ); //Raj added on 06/21/2016
							...
							System.out.println("Verification of Response JSON file for " + JSONFile + ": " + " for " +"\n" +  ErrorType +  " Expected Error Description In Input JSON: " + "=" + Value  +"\n" + " FAILED" );
							System.out.println("Verification of Response JSON file for " + JSONFile + ": " + " for " +"\n" +  ErrorType +  " Actual Error Description In Output JSON: " + "=" + outputValue  +"\n" + " FAILED" );//Raj added on 06/21/2016
...
public void verifyErrorCodes_in_json_response__with_input_excel(String propertiesFileKey ,String MyInputDataSheet , String RowIndex ){
	String rangeNum2;
	int j = context.STestName.lastIndexOf("."); //This will verify Test tag (Test name) is having value of feature file name with .feature at end in TestNG.xml. For Ex: <test name="US378794_SpecialEffects.feature" preserve-order="false">-Updated this comment on 07/19/2016 by Raj
	if (j == -1)
	{
		this.context.addResult("Reading webservice property file name ", "Reading webservice property file name ", "Invalid user story name " , "Invalid user story name ",  RunStatus.FAIL);
	}
     ...
     if(RowIndex.equalsIgnoreCase("ALL"))
	{
		...
		if(RowIndex.contains("-")){
			String[] hiphenChar=RowIndex.split("-");
			int rangeNum=Integer.parseInt(hiphenChar[1])-Integer.parseInt(hiphenChar[0]);
			for(int k=0;k<=rangeNum;k++){
				int rangeNum1 = Integer.parseInt(hiphenChar[0])+k;
				
				rangeNum2=Integer.toString(rangeNum1);
				VerifyInputOutPutErrorMeg(USname, propertiesFileKey ,MyInputDataSheet , rangeNum2);
			}
		}else if(RowIndex.contains(",")){
			String[] cammaChar=RowIndex.split(",");
			for(int n=0;n<cammaChar.length;n++){
				VerifyInputOutPutErrorMeg(USname, propertiesFileKey ,MyInputDataSheet , cammaChar[n]);
			}
		}
		else{
			VerifyInputOutPutErrorMeg(USname, propertiesFileKey ,MyInputDataSheet , RowIndex);
		}
...
//     			    inputJSONFile = updatedinputJson;//mahesh
				...
				for(int k=0;k<=rangeNum;k++){
					int rangeNum1 = Integer.parseInt(hiphenChar[0])+k;
					JSONData jsonData = new JSONData();
				    String updatedinputJson = InputJsonPath + USname + "\\" + propertiesFileKey + "_" + context.strScenarioRunId +  "_Data_Row_" + rangeNum1 + "_POST.json";
//					    inputJSONFile = updatedinputJson;
				    jsonData.setInputJSONFile(updatedinputJson);
				    jsonData.setIndex(k-1);
				    CreateAJSONWithInputData(context ,updatedinputJson ,rangeNum1-1 , ClassName, jsonData);
					...
					String[] cammaChar=RowIndex.split(",");
					for(int n=0;n<cammaChar.length;n++){
						int DataRowIndex = Integer.parseInt(cammaChar[n])-1;
						JSONData jsonData = new JSONData();
					    String updatedinputJson = InputJsonPath + USname + "\\" + propertiesFileKey + "_" + context.strScenarioRunId +  "_Data_Row_" + cammaChar[n] + "_POST.json";
//					    inputJSONFile = updatedinputJson;
					    jsonData.setInputJSONFile(updatedinputJson);
					    jsonData.setIndex(DataRowIndex);
...
//              Object obj = mapper.readValue(eventsObject.toString(), clazz); 
            ...
            IOUtils.write(prettyJson, new FileOutputStream(OutputJsonFileName), Charset.defaultCharset());
              ...
              this.context.addResult("Created API request JSON file for row: "+(DataRowIndex+1), "Successfully Created API request JSON file", "Create API request JSON file ", "Successfully Created API request JSON file " + OutputJsonFileName, RunStatus.PASS);
		}
		catch (Exception e){
				...
				if(!RowIndex.equalsIgnoreCase("ALL")){
					if(RowIndex.contains("-")){
						...
						for(int k=0;k<=rangeNum;k++){
							...
							rangeNum2=Integer.toString(rangeNum1);
							ProcessJsonInputFileWithExcelRows(propertiesFileKey,SucessorFailure, status, MyInputDataSheet,rangeNum2);
						}
					}
					else if(RowIndex.contains(",")){
						String[] cammaChar=RowIndex.split(",");
						for(int n=0;n<cammaChar.length;n++){
							...
							ProcessJsonInputFileWithExcelRows(propertiesFileKey,SucessorFailure, status, MyInputDataSheet,cammaChar[n]);
						}
					}else{
						ProcessJsonInputFileWithExcelRows(propertiesFileKey,SucessorFailure, status, MyInputDataSheet,RowIndex);
					}
				}else{ 
					...
					if(index == -1) {
						...
						for (int i = 1; i <= totalRows; i++) {
							index=i-1;
							List<String> valuesList = new ArrayList<String>();
							actualObj = dataObjects.get(index);
							url =  CommonUtility.buildURL(actualObj);
							valuesList.add(actualObj.getServiceName());
							valuesList.add(url);
							valuesList.add(actualObj.getHttpMethod());
							valuesList.addAll(actualObj.getHeaders());
...
//						//===Added by Raj on 6/15/2016=====start==Even the request was having 400, test used to display as Test Step Passed. Now changed to display fail if there is 400
//						if(res.getStatusCode()==200 && res.getStatusLine().equalsIgnoreCase("HTTP/1.1 200 OK")){
//							this.context.addResult("Request Posted sucessfully for webservice call "+propertiesFileKey +" For Data Row: "+RowIndex, "Execute webservice call: "+ webServiceParameters.getStrTestName(), "Webserivce call executed ", "Webservice call returned Status Code = " + res.getStatusCode() +  " StatusLine =  " + res.getStatusLine(), RunStatus.PASS);
//
//						}else if(res.getStatusCode()==400 && res.getStatusLine().equalsIgnoreCase("HTTP/1.1 400 Bad Request")){
//							this.context.addResult("Request Posted but responce failed for webservice "+propertiesFileKey +" For Data Row: "+RowIndex, "Execute webservice call: "+ webServiceParameters.getStrTestName(), "Webserivce call executed ", "Webservice call returned Status Code = " + res.getStatusCode() +  " StatusLine =  " + res.getStatusLine(), RunStatus.FAIL);
//
//						}
//						
...
//
//							{
//								//writeLineToFile(strJSONOutputFileName, strJsonOutput); //Raj add For Data Row: "+RowIndex on 06/15/2016
//								this.context.addResult("Generate json response file: " + propertiesFileKey+" For Data Row: "+RowIndex, "Generate json response file", "Generated json response file " + strJSONOutputFileName, strJsonOutput, RunStatus.PASS);
//							} else if (res.getStatusCode() == 503) {
//								this.context.addResult("Request Posted for WebService Call " + propertiesFileKey+" For Data Row: "+RowIndex, "To Execute the WebService : " + testname, " Service Temporarily Unavailable" + res.getStatusLine(), " Webservice Temporarily Unavailable", RunStatus.FAIL);
//							} else
//								this.context.addResult("Request Posted for WebService Call" + propertiesFileKey+" For Data Row: "+RowIndex, "To Execute the WebService : " + testname, "WebService Needs to be executed", "Error occured while execution of webservice. Error code : " + res.getStatusLine(), RunStatus.FAIL);
//						} else if (SucessorFailure.equalsIgnoreCase("Failure"))  //(SucessorFailure.equalsIgnoreCase("exception"))
//						{
//							if (ArrayUtils.contains(Invalidcodelist, "" + res.getStatusCode())) {
//								this.context.addResult("Verify response status code for " + propertiesFileKey, "verify response status code", "Status code for api call =  " + res.getStatusCode() + "\n" + strJSONOutputFileName, "api call failed as expected" + "\n\n" + strJsonOutput, RunStatus.PASS);
...
//				}
	 ...
	 public void ProcessJsonInputFileWithExcelRows(String propertiesFileKey, String SucessorFailure, String status, String MyInputDataSheet, String RowIndex){
		 	DataObject newObj = new DataObject();
			newObj.setServiceName(propertiesFileKey);
			newObj.setEnvironment(ExecutionContext.SRunAgainst);
			int index = dataObjects.indexOf(newObj);
...
//		 int RowIndex1= Integer.parseInt(RowIndex)-2;//Raj commented on 8/2/2016 for Add service. Pass header data from ServiceData sheet. No need of redusing the colum number. Check for Change
//			index=RowIndex1;
...
//			----------------------------Start------------------Raj Added on 06/16/20016----------
			...
			if(index != -1) {
				...
				URI uri = null;
				try {
					surl = new URL(webServiceParameters.getStrURI());
					uri = new URI(surl.getProtocol(), surl.getUserInfo(), surl.getHost(), surl.getPort(), surl.getPath(), surl.getQuery(), surl.getRef());
				}
				 catch (Exception e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
			...
			if (webServiceParameters.getStrRequestType().equals("POST")) {
					...
					if (webServiceParameters.getHeaders() != null) {
						...
						res = given().contentType("application/json").post(strURL, strBody);//check it for validating header parameters
				...
				if(res.getStatusCode()==200 && res.getStatusLine().equalsIgnoreCase("HTTP/1.1 200 OK")){
					...
					if(SucessorFailure.equalsIgnoreCase("Failure")){
						this.context.addResult("Request Posted sucessfully for webservice call "+propertiesFileKey +" For Data Row: "+RowIndex, "Execute webservice call: "+ webServiceParameters.getStrTestName(), "Webserivce call executed ", "Webservice call returned Status Code = " + res.getStatusCode() +  " StatusLine =  " + res.getStatusLine(), RunStatus.PASS);
					}else{
						this.context.addResult("Request Posted but responce failed for webservice "+propertiesFileKey +" For Data Row: "+RowIndex, "Execute webservice call: "+ webServiceParameters.getStrTestName(), "Webserivce call executed ", "Webservice call returned Status Code = " + res.getStatusCode() +  " StatusLine =  " + res.getStatusLine(), RunStatus.FAIL);
					}
				...
				{
					if (ArrayUtils.contains(Invalidcodelist, "" + res.getStatusCode())) {
						this.context.addResult("Verify response status code for " + propertiesFileKey+" For Row: "+RowIndex, "verify response status code", "Status code for api call =  " + res.getStatusCode() + "\n" + strJSONOutputFileName, "api call failed as expected" + "\n\n" + strJsonOutput, RunStatus.PASS);
					} else
						this.context.addResult("Execute WebService for " + propertiesFileKey+" For Row: "+RowIndex, "To Execute the WebService : " + testname, "WebService Needs to be executed", "Error occured while execution of webservice. Error code : " + res.getStatusLine(), RunStatus.FAIL);
				}
			}
	 }
	 
	 public void VerifyInputOutPutErrorMeg(String USname,String propertiesFileKey ,String MyInputDataSheet , String RowIndex){
		 List<String> ErrorCodesListToVerify = new ArrayList<String>();
	     List<String> ErrorDescListToVerify = new ArrayList<String>();
	     List<String> DevErrorCodesListToVerify = new ArrayList<String>();
	     List<String> DevErrorDescListToVerify = new ArrayList<String>();
		 int DataRowIndex = Integer.parseInt(RowIndex)-2; // Modified after implementation of JSON w/excel data
		   ...
		   if(context.datamap.get(0).get("UserErrorCodes").get(DataRowIndex) != null){
			   String[] errorCodes = context.datamap.get(0).get("UserErrorCodes").get(DataRowIndex).split("-"); //Raj has changed from '#' to '-' as the error message
			   for (int m=0; m<errorCodes.length; m++)
	           {
	        	   ErrorCodesListToVerify.add(errorCodes[m]);
	           }
		   ...
		   if(context.datamap.get(0).get("DevErrorDesc").get(DataRowIndex) != null ){
			   String[] deverrorDesc = context.datamap.get(0).get("DevErrorDesc").get(DataRowIndex).split("/"); //Raj has updated on 08/03/2016 change from '-' to '// because some error desc has -
			   
			   for (int l=0; l<deverrorDesc.length; l++)
	           {
				   System.out.println("chenged '-'"+deverrorDesc[l]);
	        	   DevErrorDescListToVerify.add(deverrorDesc[l]);
	           }
		   }
		   else{
	 ...
	 public void OutputFileMapping(String key, String value)
	    {
	    	try{
	    		Properties properties = new Properties();
	    	    //properties.load(new FileInputStream(PropertiesFile.GetEnvironmentProperty("TESTDATA_EXCEL_PATH") + "outputFileMapping.properties"));
	    		properties.load(new FileInputStream(System.getProperty("user.dir")+PropertiesFile.GetEnvironmentProperty("TESTDATA_EXCEL_PATH")+"\\outputFileMapping.properties"));
				Map<String, String> map = new HashMap<String, String>((Map)properties);
	    	    map.put(key, value);
	    	    properties.putAll(map);
	    	    //properties.store(new FileOutputStream(PropertiesFile.GetEnvironmentProperty("TESTDATA_EXCEL_PATH") + "outputFileMapping.properties"), null);
	    	    properties.store(new FileOutputStream(System.getProperty("user.dir")+PropertiesFile.GetEnvironmentProperty("TESTDATA_EXCEL_PATH")+"\\outputFileMapping.properties"), null);
	    	}catch(Exception e)
	    	{
	    		e.printStackTrace();
	    	}
