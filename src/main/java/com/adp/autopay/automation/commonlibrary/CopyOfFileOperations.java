...
public class CopyOfFileOperations
{
	ExecutionContext context;
	public static String AdminServer = PropertiesFile.GetProperty("AdminServerIP");

	public CopyOfFileOperations(ExecutionContext context){
...
//			FileFilter fileFilter = new WildcardFileFilter(Prefix+"_"+COID+"_*.log");
	...
	public static void ReplaceContentInJSONFile(ExecutionContext context,String Filename,String Fieldnames, String fieldToCheck,String fieldToReplace ) throws IOException {
		try {
			...
			File tempFolder = new File ("C:/Temp/Output" + USname );
		...
		for (int i = 0; i < names.length(); i++) {
			...
			if(dataMap.get(name) != null)
			{
				...
				if(value instanceof JSONObject) {
					List mapValues = (List) dataMap.get(name);
					...
					obj.put(name, ((List)dataMap.get(name)).get(index));
				}
...
//				BufferedWriter bw = new BufferedWriter(new FileWriter(outPutFile));
				...
				for (int i=0; i< listObj.length(); i++) {
					...
					if(obj instanceof JSONObject) {
						...
						List<Object> listObjs= getAllObjs(jsonObj.names(), jsonObj, datamap , index);
						getVerifyTheValue(listObjs, datamap, index);
